...
We can find other open-source simulation codes using high order non-oscillatory finite volume schemes for the more-general Navier-Stokes equations, aimed at similar aplications, which show a superior performance than most state-of-the-art comercial codes. Some examples are OpenSBLI [@lusher2021opensbli], HTR Solver [@di2020htr], JAX-Fluids [@bezgin2023jax], UCNS3D solver [@ANTONIADIS2022108453], URANOS [@de2023uranos], STREAmS [@bernardini2021streams], HyPar [@hypar] and [@cerisse]. Analogously, there are other open-source solvers based on  the Discontinuous Galerkin method such as HORSES3D [@FERRER2023108700], GALÆEXI [@kempf2024gal], Nek5000 [@markidis2015openacc] and NebulaSEM [@Abdi2024]. Compared to DG-FEM approaches (and hybrid DG-FEM variants), high-order finite volume methods like those in CAELUM generally offer a lower algorithmic and implementation complexity, retaining robustness and simplicity in handling shocks and discontinuities. This is done at the cost of lower formal accuracy per degree of freedom and a non-local nature which may sometimes compromise the computational efficiency, that is, they rely on wider stencils that hinder parallel scalability.
