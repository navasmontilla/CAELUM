...
We can find other open-source simulation codes using high order non-oscillatory finite volume schemes for the more-general Navier-Stokes equations, aimed at similar aplications, which show a superior performance than most state-of-the-art comercial codes. Some examples are OpenSBLI [@lusher2021opensbli], HTR Solver [@di2020htr], JAX-Fluids [@bezgin2023jax], UCNS3D solver [@ANTONIADIS2022108453], URANOS [@de2023uranos], STREAmS [@bernardini2021streams] and HyPar [@hypar]. Analogously, there are other open-source solvers based on  the Discontinuous Galerkin method such as HORSES3D [@FERRER2023108700], GALÆEXI [@kempf2024gal], Nek5000 [@markidis2015openacc] and NebulaSEM [@Abdi2024]. We can also find finite volume solvers aimed exclusively at atmospheric applications, such as the OpenFoam-based GEA [@girfoglio2023validation] and a WENO-based model for the moist atmosphere [@norman2023investigating]. Note that `CÆLUM` represents an alternative for those seeking a simpler and more academic tool, as it is designed to be pedagogical and versatile in terms of applications, though it has some limitations when considering realistic cases. It can also serve as a testbed for evaluating novel Riemann solvers and spatial reconstruction methods.
...
The example cases provided showcase a diverse suite of test problems commonly used to validate and demonstrate the capabilities of computational fluid dynamics codes. These include scalar transport problems like linear advection and Burgers' equation in 1D, 2D and 3D, compressible flow simulations such as Riemann problems, shock–bubble interactions, and a Richtmyer–Meshkov instability for turbulent mixing. Additionally, atmospheric flow cases feature colliding thermal bubbles and descending thermal plumes over complex terrain. Each case is supported by Python scripts (or Jupyter notebooks) and corresponding visual animations, making them useful for verifying the numerical solvers and exploring the underlying physical phenomena. 
