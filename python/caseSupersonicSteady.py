...
folder_case="caseSupersonicSteady/" 
...
# -------------------------------------------------
xs = 10.00   # 2 cm from inlet (adjust if desired)

# -------------------------------------------------
# Triangle geometry (30° half-angle)
# -------------------------------------------------
theta = np.deg2rad(40.0/2.0)

xa = 0.020          # cusp location (m)
ya = SizeY/2.0      # centered vertically
length_x = 0.06#0.010/np.tan(theta)    #  cm triangle length
...
for fname in files:
    ...
    plot1=ax.contourf(X, Y, Srho, 512, cmap='RdBu_r')   
    ax.set_xlabel("x") 
    ax.set_ylabel("y") 
    #ax.set_xlim([0,3]) 
    #ax.set_ylim([0,2]) 
    ...
    if (j==lf-1):
        # =====================================================
        # Shock detection: max |grad rho| per row + mask
        # =====================================================

        # ---- 1. Threshold global ----
        threshold = 0.1 * np.max(grad_rho)
        
        print(np.max(grad_rho))

        # ---- 2. Inicializar listas de puntos del choque ----
        x_shock = []
        y_shock = []

        # ---- 3. Recorrer cada fila (y constante) ----
        for j_row in range(grad_rho.shape[0]):  # eje y

            grad_row = grad_rho[j_row, :]
            
            # aplicar threshold
            valid = grad_row > threshold
            
            if np.any(valid):
                # coger el índice del máximo dentro de los válidos
                idx = np.argmax(grad_row * valid)
                
                #print(j_row,np.max(grad_row * valid))
                
                x_val = xp[idx]
                y_val = yp[j_row]
                
                x_shock.append(x_val)
                y_shock.append(y_val)
        ...
        # ---- 4. Máscara espacial (IMPORTANTE) ----
        mask_region = (
            (x_shock > x0        ) &     # evitar el vértice
            (y_shock > SizeY*0.65) &     # evitar parte baja
            (y_shock < SizeY*0.85)       # evitar borde superior
        )
        #mask_region = (x_shock > 0.0        )
        
        x_shock = x_shock[mask_region]
        y_shock = y_shock[mask_region]

        # ---- 5. Ajuste lineal ----
        coeffs = np.polyfit(x_shock, y_shock, 1)
        m = coeffs[0]
        b = coeffs[1]
        ...
        print(f"β_num= {beta_deg:.2f}º")

        # ---- 6. Línea ajustada ----
        x_fit = np.linspace(np.min(x_shock), np.max(x_shock), 100)
        y_fit = m * x_fit + b
        
        
        ax2.plot(x_fit, y_fit, 'r--', linewidth=2, label=f'β ≈ {beta_deg:.1f}°')
        ax2.scatter(x_shock, y_shock, s=5, color='yellow', label='shock points')
        ax2.legend()
