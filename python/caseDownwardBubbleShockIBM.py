...
folder_case="caseDownwardBubbleShockIBM/" 
...
modify_header_file(folder_lib+'/definitions.h', 'TYPE_REC', 1)
...
#Simulation setup
FinalTime = 35.0
DumpTime = 10.0  #for file printing
CFL = 0.45
Order = 7
...
for l in range(0,xcells): 
        for m in range(0,ycells): 
            for n in range(0,zcells):
                ...
                # Temperatura en equilibrio adiabático
                T = theta - g*zc[l,m,n]/(gamma*cv)

                # Presión
                p[l,m,n] = p0 * (T/theta)**(gamma/(gamma-1.0))

                # Densidad
                rho[l,m,n] = p[l,m,n]/(R*T)
...
for fname in files:
    ...
    levels = np.linspace(0, 3000, 6)
    #print(levels)
    ax.contour(X, Y, Sth, levels=levels,colors="k",linewidths=0.5)  
    plot1=ax.contourf(X, Y, Sth, 256, cmap='RdBu_r', vmin=-2400, vmax=3000)   
    ax.set_xlabel("x (m)")
    ax.set_ylabel("z (m)")
    ax.set_xlim([0, SizeX])
    ax.set_ylim([0, SizeZ])
    ...
    Srho=rho[:,0,:,j]
    Srho = np.ma.masked_where((Srho < 0) | np.isnan(Srho), Srho)
    
    from matplotlib.colors import LinearSegmentedColormap
    colors = [
        (0.0, (0.0, 0.0, 0.0)),      # Negro
        (0.333, (0.0, 0.0, 0.502)),  # Azul oscuro
        (0.666, (0.0, 0.502, 1.0)),  # Azul claro
    ...
    fig, ax = plt.subplots(figsize=(7,3.5))  
    #levels = np.linspace(0.1, 1.3, 256)
    #print(levels)
    #ax.contour(X, Y, Srho, levels=levels,colors="k",linewidths=0.5)  
    plot1 = ax.pcolormesh(
    X, Y, Srho,
    cmap=cmap,
    norm=LogNorm(vmin=0.1, vmax=1.2),
    shading='auto'
    )    
    ax.set_xlabel("x (m)")
    ax.set_ylabel("z (m)")
