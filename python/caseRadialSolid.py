...
for l in range(0,xcells): 
    for m in range(0,ycells):
        for n in range(0,zcells):
            ...
            yp=0.5
            zp=0.5;
            rc=np.sqrt((xc[l,m,n]-xp)*(xc[l,m,n]-xp)+(yc[l,m,n]-xp)*(yc[l,m,n]-yp)+(zc[l,m,n]-zp)*(zc[l,m,n]-zp));
            ...
            if 0.3 <= xc[l, m, n] < 0.7 and 0.3 <= yc[l, m, n] < 0.7 and 0.2 <= zc[l, m, n] < 0.3:
...
for fname in files:
    ...
    sliced_grid = grid.slice(normal=(1, 0, 0), origin=(0.5, 0.5, 0.5))
    plotter.add_mesh(sliced_grid, cmap="coolwarm", scalars="rho", opacity=1.0)
    sliced_grid = grid.slice(normal=(0, 1, 0), origin=(0.5, 0.5, 0.5))
    plotter.add_mesh(sliced_grid, cmap="coolwarm", scalars="rho", opacity=1.0)
    contour_values = -0.9
    contours = grid.contour(isosurfaces=[contour_values], scalars="rho") 
    if contours.n_points > 0:
